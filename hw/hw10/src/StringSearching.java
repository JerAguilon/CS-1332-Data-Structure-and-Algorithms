...
public class StringSearching {
    ...
    public static List<Integer> kmp(CharSequence pattern, CharSequence text) {
        if (pattern == null || pattern.length() == 0 || text == null) {
            throw new IllegalArgumentException("Neither pattern nor text can "
                    + "be null and pattern length must be nonzero");
        }
        ...
        //m - j: chars in pattern, n - i: chars in text
        while (m - j <= n - i) {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern,
            CharSequence text) {
        if (pattern == null || pattern.length() == 0 || text == null) {
            throw new IllegalArgumentException("Neither pattern nor text "
                    + "may be null. Pattern's length cannot be 0");
        }
    ...
    public static List<Integer> rabinKarp(CharSequence pattern,
        ...
        //Matches are impossible when pattern is longer than text
        if (pattern.length() > text.length()) {
            return new ArrayList<>();
        }
        ...
        for (int index = 0;
            ...
            if (index != text.length() - pattern.length()) {
                textHash = updateHash(textHash, pattern.length(),
                        text.charAt(index),
                        text.charAt(index + pattern.length()));
            }
    ...
    /**
     * Checks if two CharSequence objects are equal by looping through
     * all chars in pattern and text starting from @param start.
     *
     * @param pattern a string you're searching for in a body of text
     * @param text the body of text where you search for pattern
     * @param start the starting index to be compared in text
     * @return boolean of whether the two are equal or not
     */
    private static boolean equals(CharSequence pattern,
    ...
    public static int generateHash(CharSequence current, int length) {
        if (current == null || length < 0 || length > current.length()) {
            throw new IllegalArgumentException("Current cannot be null,"
                    + " length cannot be negative, and current's length must be"
                    + " greater than length");
        }

        int output = 0;
        for (int i = length - 1, j = 0; i >= 0; i--, j++) {
            output += current.charAt(j) * pow(BASE, i);
        }
        return output;
    }
